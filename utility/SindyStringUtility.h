#ifndef SINDY_STRING_UTILITY_H
#define SINDY_STRING_UTILITY_H
...
inline bool split(const std::string& src, std::vector<std::string>& res, const std::string& pattern, size_t offset = 0, size_t len = std::string::npos)
{
	res.clear();
	size_t Start = 0;
	size_t end = 0;
	...
	while (Start < src.size()) {
		...
		if (std::string::npos == end || res.size() >= len) {
...
inline std::vector<std::string> split(const std::string& src, const std::string& pattern, size_t len = std::string::npos)
{
