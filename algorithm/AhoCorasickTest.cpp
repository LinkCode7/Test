...
void TestCodePoint::entry()
{
	...
	initMainContents();
	initSchemaList();
	std::cout << "\n\n\n主串数量(文本图层)：" << m_contents.size() << "，模式串数量(emoji)：" << m_schema.size()
		<< std::endl;
...
void TestCodePoint::initMainContents()
{
	...
	for (const auto& strContent : arrContents) {
		...
		for (const auto& str : arrCodePoints) {
			...
			auto pair = std::make_pair(SkUnichar(atoi(result[0].data())),
				TextRange(TextIndex(atoi(result[1].data())), atoi(result[2].data())));
...
void TestCodePoint::initSchemaList()
{
	...
	std::sort(m_schema.begin(), m_schema.end(), [](auto& left, auto& right) {
		return left->m_codePoints.size() > right->m_codePoints.size();
		});
}
...
void TestCodePoint::initContents(std::vector<std::string>& arrInfo)
{
	...
	arrInfo.emplace_back(
		"史學家🔅🔱ff♻:21490,0,3;23416,3,6;23478,6,9;128261,9,13;128305,13,17;102,17,18;102,18,19;9851,19,22;");
}
