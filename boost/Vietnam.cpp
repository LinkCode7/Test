...
int __main(int argc, _TCHAR* argv[])
{
    ...
    // コマンドライン引数処理
    Arguments args;
    desc.add_options()(k_in_db, uh::tvalue<uh::tstring>(&args.in_db), "[必須]インプットDB")(
        k_log, uh::tvalue<uh::tstring>(&args.log),
        "[必須]ログ")(k_out_db, uh::tvalue<uh::tstring>(&args.out_db),
                      "[必須]アウトプットDB")(k_mesh_db, uh::tvalue<uh::tstring>(&args.mesh_db),
                                              "[道路変換時使用]メッシュDB")(k_poi_code, uh::tvalue<uh::tstring>(&args.poi_code),
                                                                            "[必須]poiコードファイル")(k_chainmastername_code,
                                                                                                       uh::tvalue<uh::tstring>(
                                                                                                           &args.chainmaster_code),
                                                                                                       "[必須]"
                                                                                                       "chainmasterコードファイ"
                                                                                                       "ル")(k_adminid_list,
                                                                                                             uh::tvalue<uh::tstring>(
        ...
        (k_site,
         "背景ポリゴン変換モード")(k_building,
                                   "建物変換モード")(k_admin,
                                                     "行政界変換モード")(k_road,
                                                                         "道路変換モード")(k_poi, "POI変換モード")(k_hnp,
                                                                                                                   "ハウスナンバーポイント")
    ...
    if (vm.count(k_site) != 0)
    {
        args.site = true;
    }
    if (vm.count(k_building) != 0)
    {
        args.building = true;
    }
    if (vm.count(k_admin) != 0)
    {
        args.admin = true;
    }
    if (vm.count(k_road) != 0)
    {
        args.road = true;
    }
    if (vm.count(k_poi) != 0)
